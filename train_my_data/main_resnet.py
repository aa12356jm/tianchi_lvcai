...
# 保存最新模型以及最优模型
def save_checkpoint(state, is_best, is_lowest_loss, file_name):
...
# 验证函数
def validate(val_loader, model, criterion,print_freq,best_precision,lowest_loss):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
...
# 测试函数
def test(test_loader, model,file_name):
...
def adjust_learning_rate(lr,lr_decay,model,weight_decay):
    lr = lr / lr_decay
...
def main():
    ...
    resume = False
    # 创建模型
    #model = model_v4.v4(num_classes=12) #baseline带的

    ######################################创建模型：cym###########################
    mode1_ft_res = torchvision.models.resnet152(pretrained=True)
    # 有时候我们加载了训练模型后，只想调节最后的几层，其他层不训练。其实不训练也就意味着不进行梯度计算，
    # PyTorch中提供的requires_grad使得对训练的控制变得非常简单。
    for param in mode1_ft_res.parameters():
         param.requires_grad = False

    num_fc = mode1_ft_res.fc.in_features
    mode1_ft_res.avgpool = nn.AdaptiveAvgPool2d(1)#自适应池化层

    #全连接层
    mode1_ft_res.fc = nn.Sequential(
    ...
    model = torch.nn.DataParallel(mode1_ft_res).cuda()
    ...
    if resume:
        if os.path.isfile(resume):
            ...
            if start_epoch in np.cumsum(stage_epochs)[:-1]:
                stage += 1
                optimizer = adjust_learning_rate(lr,lr_decay,model,weight_decay)
    ...
    #optimizer = optim.Adam(model.fc.parameters(), lr, weight_decay=weight_decay, amsgrad=True)

    if evaluate:
        validate(val_loader, model, criterion,print_freq,best_precision,lowest_loss)
    else:
        for epoch in range(start_epoch, total_epochs):
            # train for one epoch
            train(train_loader, model, criterion, optimizer, epoch, print_freq)
            # evaluate on validation set
            precision, avg_loss = validate(val_loader, model, criterion,print_freq,best_precision,lowest_loss)
            ...
            save_checkpoint(state, is_best, is_lowest_loss, file_name)
